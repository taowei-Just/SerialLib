...
public class Verify {
    ...
    public static byte orCheck(byte[] bytes) throws Exception {
        int b = 0;
    ...
    public static byte orCheck(byte[] bytes, int offst, int len) throws Exception {
        byte[] buff = new byte[len];
        System.arraycopy(bytes, offst, buff, 0, len);
        return orCheck(buff);
    }
